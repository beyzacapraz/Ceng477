...
vec3 kd = vec3(1.0 , 1.0 , 0.2);     // diffuse reflectance coefficient
vec3 ka = vec3(0.7, 0.3, 0.3);   // ambient reflectance coefficient
...
void main(void)
{
	...
	vec3 yellowishColor = vec3(1.0, 1.0, 0.0);
    vec3 finalDiffuseColor = diffuseColor;
	
	vec3 specularColor = I * ks * pow(max(0, NdotH), 100);
	vec3 ambientColor = Iamb * ka;

	fragColor = vec4(finalDiffuseColor + specularColor + ambientColor, 1);
